...
function flatten(input) {
    return input.reduce((accu, entry) => accu.concat(Array.isArray(entry) ? entry : [entry]), []);
}
...
function makeAnsiCode(open, close) {
    return { open: escapeCode(open), close: escapeCode(close) };
}
exports.makeAnsiCode = makeAnsiCode;
